...
export class PriceCacheProcessor extends WorkerHost {
    ...
    async process(job: Job<UpdateSingleJobData | UpdateBatchJobData | undefined>): Promise<any> {
    ...
    private async handleUpdateSingle(job: Job<UpdateSingleJobData>): Promise<{ success: boolean }> {
        ...
        try {
            ...
            this.logger.debug(`Updated price cache for tour ${tourId}: min=${minPrice}, max=${maxPrice}`);
            return { success: true };
        } catch (error) {
            this.logger.error(`Failed to update price cache for tour ${tourId}`, error);
            throw error;
        }
    }

    private async handleUpdateAll(): Promise<{ updated: number; failed: number }> {
        ...
        for (const tour of tours) {
            try {
                ...
                this.logger.error(`Failed to update price cache for tour ${tour.id}`, error);
                failed++;
            }
        }

        this.logger.log(`Batch price cache update completed: ${updated} updated, ${failed} failed`);
        return { updated, failed };
    }

    private async handleUpdateBatch(job: Job<UpdateBatchJobData>): Promise<{ updated: number }> {
    ...
    private calculatePrices(tour: TourEntity): { minPrice: number | undefined; maxPrice: number | undefined } {

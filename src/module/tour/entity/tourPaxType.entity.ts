...
export class TourPaxTypeEntity extends BaseEntityTimestamp {
    ...
    @OneToMany(() => TourVariantPaxTypePriceEntity, (tour_variant_pax_type_price) => tour_variant_pax_type_price.pax_type)
    ...
    @ApiProperty({ description: 'Danh sách các mục đặt tour có loại khách này' })
    booking_items: BookingItemEntity[];

    @OneToMany(() => BookingPassengerEntity, (booking_passenger) => booking_passenger.pax_type)
    @ApiProperty({ description: 'Danh sách các hành khách có loại khách này' })

...
export class TourPriceRuleEntity extends BaseEntityTimestamp {
    ...
    @ApiProperty({ description: 'Bitmask 7-bit (CN..T7); 127 = áp dụng mọi ngày' })
    weekday_mask: number;
    ...
    @ApiProperty({ description: 'absolute: ghi đè; delta: cộng/trừ so với base' })
    ...
    @ManyToOne(() => TourVariantEntity, (tour_variant) => tour_variant.tour_price_rules, { nullable: false })
    ...
    @OneToMany(() => TourRulePaxTypePriceEntity, (tour_rule_pax_type_price) => tour_rule_pax_type_price.tour_price_rule)

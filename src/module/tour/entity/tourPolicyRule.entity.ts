...
export class TourPolicyRuleEntity extends BaseEntityTimestamp {
    ...
    @ApiProperty({ description: 'Nếu hủy trước X giờ so với start thì áp fee_pct tương ứng' })
    before_hours: Number;

    @Column({
        type: 'tinyint',
    })
    @ApiProperty({ description: 'Tỷ lệ phí hủy (%) áp cho khoảng thời gian này' })
    fee_pct: number;
    ...
    @ManyToOne(() => TourPolicyEntity, (tour_policy) => tour_policy.tour_policy_rules, { nullable: false })
    @JoinColumn({ name: 'tour_policy_id', referencedColumnName: 'id' })
    @ApiProperty({ description: 'Chính sách hủy/fee' })

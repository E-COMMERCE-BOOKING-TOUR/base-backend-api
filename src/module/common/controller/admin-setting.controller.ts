...
export class AdminSettingController {
    ...
    @Post('upload')
    @ApiOperation({ summary: 'Upload media file (logo, favicon, background, banner) lên Cloudinary' })
    ...
    @ApiBody({
        schema: {
            ...
            properties: {
                ...
                type: { type: 'string', enum: ['logo', 'favicon', 'background', 'banner'] },
            },
        },
    })
    @UseInterceptors(
        FileInterceptor('file', {
            fileFilter: (req, file, cb) => {
                if (!file.mimetype.match(/\/(jpg|jpeg|png|gif|ico|svg|webp)$/)) {

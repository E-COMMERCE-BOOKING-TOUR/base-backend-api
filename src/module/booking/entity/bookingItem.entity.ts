...
export class BookingItemEntity extends BaseEntityTimestamp {
    ...
    @ManyToOne(() => BookingEntity, (booking) => booking.booking_items, { nullable: false })
    @JoinColumn({ name: 'booking_id', referencedColumnName: 'id' })
    @ApiProperty({ description: 'Đơn đặt tour' })
    booking: BookingEntity;

    @ManyToOne(() => TourVariantEntity, (variant) => variant.booking_items, { nullable: false })
    @JoinColumn({ name: 'variant_id', referencedColumnName: 'id' })
    @ApiProperty({ description: 'Biến thể tour' })
    variant: TourVariantEntity;

    @ManyToOne(() => TourPaxTypeEntity, (pax_type) => pax_type.booking_items, { nullable: false })
    @JoinColumn({ name: 'pax_type_id', referencedColumnName: 'id' })
    @ApiProperty({ description: 'Loại khách' })
    pax_type: TourPaxTypeEntity;

    @ManyToOne(() => TourSessionEntity, (tour_session) => tour_session.booking_items, { nullable: false })
    ...
    @OneToMany(() => BookingPassengerEntity, (booking_passenger) => booking_passenger.booking_item)
    @ApiProperty({ description: 'Danh sách các hành khách của mục đặt tour này' })

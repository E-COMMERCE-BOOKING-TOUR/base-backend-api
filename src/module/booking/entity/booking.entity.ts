...
export class BookingEntity extends BaseEntityTimestamp {
    ...
    @ManyToOne(() => CurrencyEntity, (currency) => currency.bookings, { nullable: false })
    @JoinColumn({ name: 'currency_id', referencedColumnName: 'id' })
    @ApiProperty({ description: 'Tiền tệ đặt tour' })
    currency: CurrencyEntity;

    @ManyToOne(() => PaymentInfomationEntity, (payment_information) => payment_information.bookings, { nullable: false })
    ...
    @OneToOne(() => TourInventoryHoldEntity, (tour_inventory_hold) => tour_inventory_hold.booking, { nullable: false })
    ...
    @ManyToOne(() => BookingPaymentEntity, (booking_payment) => booking_payment.bookings, { nullable: false })

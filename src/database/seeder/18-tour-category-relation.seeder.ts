...
export default class TourCategoryRelationSeeder implements Seeder {
    async run(dataSource: DataSource): Promise<void> {
        ...
        if (tours.length === 0 || categories.length === 0) {
            console.log(
                '⚠️ No tours or categories found, skipping relation seeder',
            );
            return;
        }
        ...
        const categoryMap: { [key: string]: string[] } = {
            ...
            'halong-bay-luxury-cruise-2days': [
                'Cruise Tours',
                'Luxury Tours',
                'Adventure Tours',
            ],
            ...
            'phu-quoc-3-days-beach': [
                'Beach & Island Tours',
                'Luxury Tours',
                'Family Tours',
            ],
            'dalat-city-tour-flower-gardens': [
                'City Tours',
                'Cultural Tours',
                'Photography Tours',
            ],

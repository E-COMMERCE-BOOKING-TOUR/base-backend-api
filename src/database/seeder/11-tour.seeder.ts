...
export default class TourSeeder implements Seeder {
    async run(dataSource: DataSource): Promise<void> {
        ...
        const thb = await currencyRepository.findOne({ where: { symbol: 'THB' } });
        
        const suppliers = await supplierRepository.find({ where: { status: 'active' } });
        ...
        const hanoi = await divisionRepository.findOne({ where: { code: 'HN' } });
        const danang = await divisionRepository.findOne({ where: { code: 'DN' } });
        const nhatrang = await divisionRepository.findOne({ where: { code: 'KH' } });
        const phuquoc = await divisionRepository.findOne({ where: { code: 'PQ' } });
        const dalat = await divisionRepository.findOne({ where: { code: 'LD' } });
        const halong = await divisionRepository.findOne({ where: { code: 'QN' } });
        const sapa = await divisionRepository.findOne({ where: { code: 'LC' } });
        const hoian = await divisionRepository.findOne({ where: { code: 'QNA' } });
        ...
        const tours = [
            ...
            },
            {
                ...
                summary: 'Two-day adventure through Mekong Delta with floating markets, homestay experience, and cultural activities.',
            ...
            },
            {
                ...
                description: '<h2>Journey into Vietnam War history</h2><p>Visit the legendary Cu Chi Tunnels, an incredible underground network used by Vietnamese guerrillas during the war. Crawl through the tunnels and learn about ingenious survival techniques.</p>',
                summary: 'Half-day tour to the historic Cu Chi Tunnels with expert war historian guide.',
            ...
            // Hanoi Tours
            {
                ...
                description: '<h2>Discover Hanoi\'s ancient streets and delicious cuisine</h2><p>Walk through the historic Old Quarter with 36 ancient streets. Sample authentic Vietnamese street food including pho, banh mi, egg coffee, and more.</p>',
                summary: 'Evening walking tour through Hanoi Old Quarter with street food tasting experience.',
            ...
            },
            {
                ...
                description: '<h2>Cruise through UNESCO World Heritage Site</h2><p>Experience the magical beauty of Halong Bay aboard a luxury cruise. Enjoy kayaking, swimming, cave exploration, and fresh seafood. Watch sunset and sunrise over limestone karsts.</p><p>Includes: Luxury cabin, all meals, activities, and professional cruise staff.</p>',
                summary: 'Premium overnight cruise in Halong Bay with full-service amenities and activities.',
            ...
            },
            {
                ...
                description: '<h2>Trek through stunning rice terraces and ethnic villages</h2><p>Hike through spectacular mountain scenery, visit ethnic minority villages, stay with local families, and experience traditional highland culture.</p><p>Visit Cat Cat, Y Linh Ho, Lao Chai, Ta Van villages. Moderate difficulty trekking.</p>',
                summary: 'Three-day trekking adventure in Sapa with homestay and cultural immersion.',
            ...
            // Da Nang & Hoi An Tours
            {
                ...
                description: '<h2>Explore the charming UNESCO town of Hoi An</h2><p>Walk through ancient streets, visit historic merchant houses, Japanese Bridge, and create your own traditional lantern. Evening river cruise with lantern release included.</p>',
                summary: 'Full day tour in Hoi An with lantern workshop and evening river activities.',
            ...
            },
            {
                ...
                description: '<h2>Visit the famous Golden Bridge and Ba Na Hills</h2><p>Take the cable car to Ba Na Hills, walk on the iconic Golden Bridge held by giant hands, explore French Village, and enjoy Fantasy Park amusement rides.</p>',
                summary: 'Day trip to Ba Na Hills featuring the stunning Golden Bridge and entertainment park.',
            ...
            },
            {
                ...
                description: '<h2>Explore natural caves and Buddhist sanctuaries</h2><p>Climb the five Marble Mountains with stunning caves, pagodas, and viewpoints. Visit Lady Buddha statue at Linh Ung Pagoda on Monkey Mountain.</p>',
                summary: 'Half-day tour visiting Da Nang\'s spiritual and natural landmarks.',
            ...
            // Central Vietnam Tours
            {
                ...
                description: '<h2>Journey through Vietnam\'s royal history</h2><p>Visit the Imperial Citadel, Forbidden Purple City, and majestic royal tombs along Perfume River. Experience royal court music and traditional Hue cuisine.</p>',
                summary: 'Full day tour exploring Hue\'s imperial heritage and royal architecture.',
            ...
            // Beach Tours
            {
                ...
                description: '<h2>Explore beautiful islands and coral reefs</h2><p>Visit Hon Mun, Hon Tam, and Hon Mieu islands. Snorkel in crystal-clear waters, enjoy fresh seafood lunch on beach, and relax on pristine sandy shores.</p>',
                summary: 'Full day boat tour visiting multiple islands with snorkeling and beach activities.',
            ...
            },
            {
                ...
                description: '<h2>Relax on Vietnam\'s most beautiful island</h2><p>Enjoy pristine beaches, explore fishing villages, visit pepper farms, snorkel at An Thoi archipelago, and watch stunning sunsets. Resort accommodation included.</p>',
                summary: 'Three-day beach vacation package in Phu Quoc with activities and resort stay.',
            ...
            // Highland Tours
            {
                ...
                description: '<h2>Discover the City of Eternal Spring</h2><p>Visit Dalat Flower Park, Crazy House, Bao Dai Summer Palace, and Linh Phuoc Pagoda. Enjoy cool mountain air and French colonial architecture. Coffee plantation tour included.</p>',
                summary: 'Full day exploring Dalat\'s gardens, palaces, and unique attractions.',
            ...
            // Draft/Inactive Tours
            {
                ...
                description: '<h2>Cycle through Halong Bay on land</h2><p>This tour is currently being developed.</p>',
                summary: 'Cycling tour through Ninh Binh countryside - Coming soon',
        ...
        for (const tour of tours) {
            const exists = await tourRepository.findOne({ where: { slug: tour.slug } });
            if (!exists) {
